...
def image_transformation_network(content_image):
    ...
    conv4, _ = get_deconv_layer(res5, filter_height=3, filter_width=3, n_input_channels=128,
                                n_output_channels=(1, 128, 128, 64), stride=[1, 2, 2, 1])
    conv5, _ = get_deconv_layer(conv4, filter_height=3, filter_width=3, n_output_channels=(1, 256, 256, 32),
                                n_input_channels=64, stride=[1, 2, 2, 1])
    conv6, _ = get_conv_layer(conv5, filter_height=9, filter_width=9, n_channels=3, n_input_channels=32)
